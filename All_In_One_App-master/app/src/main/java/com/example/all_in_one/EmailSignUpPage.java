...
public class EmailSignUpPage extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        signUpButton.setOnClickListener(v -> {
            ...
            if (email.isEmpty()) {
                emailField.setError("You cannot leave the e-mail field empty!");
            } else {
                if (password.isEmpty()) {
                    passwordField.setError("You cannot leave the password field empty!!");
                }
            }
            ...
            firebaseAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                ...
                public void onComplete(@NonNull @org.jetbrains.annotations.NotNull Task<AuthResult> task) {
                    if (task.isSuccessful()) {
                        Toast.makeText(EmailSignUpPage.this, "Signed up successfully! Continue to Login.", Toast.LENGTH_SHORT).show();
                        progressBar.setVisibility(View.INVISIBLE);
                        Intent intent = new Intent(EmailSignUpPage.this, EmailLoginPage.class);
                        startActivity(intent);
                        finish();
                    } else {
                        Toast.makeText(EmailSignUpPage.this, "Invalid data!", Toast.LENGTH_SHORT).show();
                        progressBar.setVisibility(View.INVISIBLE);
